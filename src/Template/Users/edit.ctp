			...
			<?= $this->Form->create(NULL, ['class' => 'form-horizontal', 'id' => 'formulario']); ?>
				...
				<div class="form-group">
					...
					<div class="col-md-10 combobox">
						<select class="form-control" id="tipoU" disabled>
						...
						<input type="hidden" name="tipoUser" value=<?=$formData['tipoUser']?>>
					</div>
				</div>
						...
						<select name='departamento' class="form-control" id="dpto" <?php echo (isset($apartments)) ? '': 'disabled' ;?> >
							<?php
								...
								foreach ($apartments as $apart) {
									echo '<option value='.$apart->id;
									echo (isset($formData)&&$formData['depto']==$apart->id)?' selected':'';
									echo '>'.$apart->num.'</option>';
								}
							?>
						</select>
					</div>
